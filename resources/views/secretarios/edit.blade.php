...
@section('title', 'Editar Secretario')
@section('content_header')
    <h1><i class="fas fa-edit"></i> Editar Secretario</h1>
@stop
@section('content')
    <div class="card shadow-lg">
        <div class="card-body">
            <form method="POST" action="{{ route('secretarios.update', $secretario->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <!-- Primera columna -->
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="nombre1" id="nombre" class="form-control" 
                                   value="{{ $secretario->nombre1 }}" placeholder="Ingrese el primer nombre">
                        </div>
                        <div class="form-group">
                            <label for="nombre2"><i class="fas fa-user"></i> Segundo Nombre:</label>
                            <input type="text" name="nombre2" id="nombre2" class="form-control" 
                                   value="{{ $secretario->nombre2 }}" placeholder="Ingrese el segundo nombre">
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" name="apellidop" id="apellidop" class="form-control" 
                                   value="{{ $secretario->apellidop }}" placeholder="Ingrese el apellido paterno">
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" name="apellidom" id="apellidom" class="form-control" 
                                   value="{{ $secretario->apellidom }}" placeholder="Ingrese el apellido materno">
                        </div>
                        <div class="form-group">
                            <label for="email"><i class="fas fa-envelope"></i> Email:</label>
                            <input type="email" name="email" id="email" class="form-control" 
                                   value="{{ $secretario->email }}" placeholder="Ingrese el correo electrónico">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="sexo" id="sexo" class="form-control">
                                ...
                                <option value="M" {{ $secretario->sexo == 'M' ? 'selected' : '' }}>Masculino</option>
                                <option value="F" {{ $secretario->sexo == 'F' ? 'selected' : '' }}>Femenino</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="dni"><i class="fas fa-id-card"></i> Cédula / Pasaporte:</label>
                            <input type="text" class="form-control" id="dni" name="dni" 
                                   value="{{ $secretario->dni }}" placeholder="Ingrese la cédula o pasaporte" required>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="seccion_id" id="seccion_id" class="form-control">
                                ...
                                @foreach($secciones as $seccion)
                                    <option value="{{ $seccion->id }}" 
                                        {{ $secretario->seccion_id == $seccion->id ? 'selected' : '' }}>
                                        {{ $seccion->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <div id="preview-container" class="mt-3 text-center">
                                <img id="preview-image" 
                                     src="{{ $secretario->image ? asset('storage/' . $secretario->image) : '#' }}" 
                                     alt="Vista previa" 
