...
<div class="row py-2" style="background: #f8fafc;">
    <div class="col-md-3 form-group text-center">
        ...
        <img id="previewImage"
            src="{{ isset($postulante) && $postulante->imagen ? asset('storage/' . $postulante->imagen) : asset('images/default-avatar.jpg') }}"
    ...
    <div class="col-md-3 form-group">
        ...
        <input type="text" name="dni" class="form-control" value="{{ old('dni', $postulante->dni ?? '') }}"
            {{ isset($postulante->dni) && $postulante->dni !== '' ? 'readonly' : '' }} required>
        @error('dni')
...
<div class="row py-2" style="background: #f8fafc;">
    ...
    <div class="col-md-3 form-group">
        ...
        <input type="email" name="correo_electronico" class="form-control"
            value="{{ old('correo_electronico', $postulante->correo_electronico ?? '') }}" required>
...
<div class="row py-2" style="background: #f8fafc;">
    <div class="col-md-3 form-group">
        ...
        <input type="text" name="telefono_convencional" class="form-control"
            value="{{ old('telefono_convencional', $postulante->telefono_convencional ?? '') }}">
    </div>
    <div class="col-md-3 form-group">
    ...
    <div class="col-md-2 form-group">
        ...
        <select name="sexo" class="form-control" required>
            <option value="M" {{ old('sexo', $postulante->sexo ?? '') == 'M' ? 'selected' : '' }}>Hombre</option>
            <option value="F" {{ old('sexo', $postulante->sexo ?? '') == 'F' ? 'selected' : '' }}>Mujer</option>
        </select>
    </div>
    <div class="col-md-2 form-group">
        <label for="tipo_sangre"><i class="fas fa-tint"></i> Tipo de Sangre:</label>
        <input type="text" name="tipo_sangre" class="form-control"
            value="{{ old('tipo_sangre', $postulante->tipo_sangre ?? '') }}">
    </div>
</div>
...
<div class="row py-2" style="background: #f8fafc;">
    <div class="col-md-3 form-group">
        ...
        <input type="text" name="nacionalidad" class="form-control"
            value="{{ old('nacionalidad', $postulante->nacionalidad ?? '') }}">
    </div>
    <div class="col-md-3 form-group">
        <label for="libreta_militar">Libreta Militar:</label>
        <input type="text" name="libreta_militar" class="form-control"
            value="{{ old('libreta_militar', $postulante->libreta_militar ?? '') }}">
    </div>
    <div class="col-md-2 form-group">
        <label for="discapacidad">¿Discapacidad?</label>
        <select name="discapacidad" class="form-control" required>
            <option value="No"
                {{ old('discapacidad', $postulante->discapacidad ?? '') == 'No' ? 'selected' : '' }}>No</option>
            <option value="Si"
                {{ old('discapacidad', $postulante->discapacidad ?? '') == 'Si' ? 'selected' : '' }}>Sí</option>
        </select>
    </div>
</div>
...
<div class="row py-2" style="background: #f8fafc;">
    <div class="col-md-3 form-group" id="divPorcentajeDiscapacidad" style="display: none;">
        ...
        <input type="number" name="porcentaje_discapacidad" class="form-control" min="0" max="100"
            value="{{ old('porcentaje_discapacidad', $postulante->porcentaje_discapacidad ?? '') }}">
    </div>
    <div class="col-md-3 form-group" id="divCodigoConadis" style="display: none;">
        <label for="codigo_conadis">Código CONADIS:</label>
        <input type="text" name="codigo_conadis" class="form-control"
            value="{{ old('codigo_conadis', $postulante->codigo_conadis ?? '') }}">
    </div>
    <div class="col-md-3 form-group" id="divTipoDiscapacidad" style="display: none;">
        ...
        <input type="text" name="tipo_discapacidad" class="form-control"
            value="{{ old('tipo_discapacidad', $postulante->tipo_discapacidad ?? '') }}">
    </div>
</div>
