            ...
            <div class="col-md-10">
                <div class="card shadow-lg">
                    ...
                    <div class="card-body bg-light">
                        <form id="formPostulacion" action="{{ route('postulaciones.store') }}" method="POST"
                            ...
                            <div class="row mb-4">
                                <div class="col-12 form-group">
                                    <label for="maestria_id" class="font-weight-bold text-primary">
                                        <i class="fas fa-graduation-cap"></i> Maestría a Postular:
                                    </label>
...
<script>
    $(function() {
        ...
        const divTipoDiscapacidad = document.getElementById('divTipoDiscapacidad');

        if (discapacidadSelect) {
            discapacidadSelect.addEventListener('change', function() {
                const mostrar = this.value === 'Si';
                divPorcentajeDiscapacidad.style.display = mostrar ? 'block' : 'none';
                divCodigoConadis.style.display = mostrar ? 'block' : 'none';
                divTipoDiscapacidad.style.display = mostrar ? 'block' : 'none';
            });
        }
        ...
        $('#formPostulacion').on('submit', function(e) {
            ...
            // Validación manual usando la API del navegador
            if (!form.checkValidity()) {
                form.reportValidity(); // Muestra los mensajes nativos
                return;
            }

            const correo = $('input[name="correo_electronico"]').val();
            const dni = $('input[name="dni"]').val();

            Swal.fire({
                title: '¿Los datos son correctos?',
                html: `<strong>Correo:</strong> ${correo}<br><strong>Cédula/Pasaporte:</strong> ${dni}`,
                icon: 'question',
